...
export function StoreFilters({ stores, onFilterChange }: StoreFiltersProps) {
    // Initialize with empty arrays to show all stores by default
    const [selectedCity, setSelectedCity] = useState<string[]>([]);
    const [selectedFormat, setSelectedFormat] = useState<string[]>([]);
    const [searchQuery, setSearchQuery] = useState('');

    const cities = Array.from(new Set(stores.map(store => store.city))).sort();
    const formats = Array.from(new Set(stores.map(store => store.newFormat))).sort();

    useEffect(() => {
        // Only filter if there are actual selections or search query
        const filteredStores = stores.filter(store => {
            const cityMatch = selectedCity.length === 0 || selectedCity.includes(store.city);
            const formatMatch = selectedFormat.length === 0 || selectedFormat.includes(store.newFormat);
            const nameMatch = searchQuery === '' || store.name.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    const handleFilterChange = (type: 'city' | 'format', value: string) => {
        if (type === 'city') {
            setSelectedCity(prev =>
                prev.includes(value) ? prev.filter(city => city !== value) : [...prev, value],
            );
        } else {
            setSelectedFormat(prev =>
                prev.includes(value) ? prev.filter(format => format !== value) : [...prev, value],
            );
        }
    };
